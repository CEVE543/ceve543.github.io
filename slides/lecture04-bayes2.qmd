...
Here $\theta=1$ and we want to know $\Pr\left\{\theta = 1 \mid y=1 \right\}$.
...
1. Step 1: $\Pr\left\{ y = 1 \right\}$
    ...
    1. $\Pr\left\{y=1\right\} = 0.999 \times 0.99 + 0.001 \times 0.01$
...
histogram(θ_samples; normalize=:pdf, label="Samples", legend=:topleft, xlabel=L"$θ$", ylabel=L"$p(θ | y)$")
...
```{julia}
histogram(coin_chain[:θ]; label="Samples", normalize=:pdf, legend=:topleft, xlabel=L"$θ$", ylabel=L"$p(θ | y)$")
plot!(closed_form; label="Exact Posterior", linewidth=3)
...
1. To sample from the prior, we can just pass in `missing` data.
